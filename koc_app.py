...
# --- SAYFA AYARLARI ---
st.set_page_config(page_title="KOÇ HALİL ŞAHAN",
layout="wide")
...
def set_bg(main_bg):
    if os.path.exists(main_bg):
        ...
        st.markdown(f"""<style>
         .stApp {{
background: url("data:image/png;base64,{b64}"); background-size:
cover; background-attachment: fixed; }}
         .stApp::before
{{ content: ""; position: absolute; top: 0; left: 0; width: 100%;
height: 100%; background-color: rgba(0, 0, 0, 0.75); z-index: 0; }}
         .main
.block-container {{ position: relative; z-index: 10; }}
         h1, h2, h3, p,
label {{ color: white !important; text-shadow: 2px 2px 10px rgba(0,0,0,1)
!important; }}
         .main-title {{
font-size: 4rem !important; font-weight: 800; text-align: center; color: white;
}}
...
PROGRAM_DOSYASI = "programlar.csv" # YENİ DOSYA
 
# KOLON TANIMLARI
KILO_KOLON = ['Tarih', 'Öğrenci Adı', 'Kilo', 'Not']
OLCU_KOLON = ['Tarih', 'Öğrenci Adı', 'Kilo', 'Boy', 'Omuz',
'Kalça', 'Baldır', 'Üst Kol', 'Alt Kol', 'Göğüs', 'Bel', 'Bacak']
BESLENME_KOLON = ['Tarih', 'Öğrenci Adı', 'Öğünler']
PROGRAM_KOLON = ['Öğrenci Adı', 'Beslenme Programı', 'Antrenman Programı', 'Tarih'] # YENİ KOLONLAR
...
# --- KULLANICI SİSTEMİ ---
KULLANICILAR = {"halil": "sahan123",
"emrecan": "emrecan2026", "ceyda":
"ceyda2026", "umuttatar": "tatar2026"}
...
if st.session_state.user is None:
    ...
    if current_user == "halil":
        with st.sidebar:
            ...
            menu = st.radio("MENÜ", ["🏠 Genel Tablo", "📊 Detaylı Analiz", "🥗 Beslenme", "⚖️ Günlük Kilolar", "📏 Ölçü Kayıtları", "📝 Program Yükle", "🗑️ Veri Sil"])
        ...
        elif menu == "📝 Program Yükle":
            st.title("Öğrenciye Program Tanımla 📝")
            sporcular = [k for k in KULLANICILAR.keys() if k != "halil"]
            secilen_ogrenci = st.selectbox("Öğrenci Seç:", sporcular)
            
            df_prog = veriyi_yukle(PROGRAM_DOSYASI, PROGRAM_KOLON)
            # Mevcut programı çekelim
            mevcut = df_prog[df_prog['Öğrenci Adı'] == secilen_ogrenci]
            eski_beslenme = mevcut['Beslenme Programı'].values[0] if not mevcut.empty else ""
            eski_antrenman = mevcut['Antrenman Programı'].values[0] if not mevcut.empty else ""
            
            bes_yeni = st.text_area("Beslenme Programı (Metin veya Link)", value=eski_beslenme, height=200)
            ant_yeni = st.text_area("Antrenman Programı (Metin veya Link)", value=eski_antrenman, height=200)
            
            if st.button("PROGRAMI KAYDET VE GÖNDER 🔥"):
                yeni_satir = pd.DataFrame([[secilen_ogrenci, bes_yeni, ant_yeni, date.today()]], columns=PROGRAM_KOLON)
                # Eskiyi sil yeniyi ekle
                df_prog = df_prog[df_prog['Öğrenci Adı'] != secilen_ogrenci]
                df_prog = pd.concat([df_prog, yeni_satir])
                github_a_kaydet(PROGRAM_DOSYASI, df_prog)
