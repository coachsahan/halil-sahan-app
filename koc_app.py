...
if st.session_state.user is None:
    ...
    if current_user == "halil":
        ...
        menu = st.sidebar.radio("MENÜ", ["🏠 Genel Tablo", "📊 Analiz", "⚖️ Kilolar", "📏 Ölçüler", "🗑️ Sil"])
        if st.sidebar.button("Çıkış Yap"):
            st.session_state.user = None
            st.rerun()

        if menu == "🏠 Genel Tablo" or menu == "⚖️ Kilolar":
        ...
        with tab2:
            with st.form("o"):
                ...
                oka = c2.number_input("Kalça", step=0.1)
                hbal = c1.number_input("Baldır", step=0.1)
                ouk = c2.number_input("Üst Kol", step=0.1)
                if st.form_submit_button("GÖNDER"):
                    df = veriyi_yukle(OLCU_DOSYASI, OLCU_KOLON)
                    yeni = pd.DataFrame([[date.today(), current_user, ok, ob, oo, oka, hbal, ouk, 0, 0, 0, 0]], columns=OLCU_KOLON)
