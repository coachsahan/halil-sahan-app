...
if st.session_state.user is None:
    ...
    # --- ÖĞRENCİ PANELİ ---
        ...
        with tab3:
            ...
            filtre = df_k[df_k['Öğrenci Adı'].str.lower() == current_user.lower()].sort_values("Tarih")
            if not filtre.empty:
                fig = px.line(filtre, x="Tarih", y="Kilo", title="Kilo Gelişim Grafiğin", markers=True)
                fig.update_layout(template="plotly_dark")
                st.plotly_chart(fig, use_container_width=True)
                st.markdown("### ⚖️ Kilo Geçmişin")
                st.table(fark_hesapla(filtre))
            
            st.markdown("---")
            st.markdown("### 📏 Ölçü Geçmişin")
            olcu_filtre = df_o[df_o['Öğrenci Adı'].str.lower() == current_user.lower()]
            if not olcu_filtre.empty:
                st.table(fark_hesapla(olcu_filtre))
            else:
                st.info("Henüz ölçü kaydın bulunmuyor.")
